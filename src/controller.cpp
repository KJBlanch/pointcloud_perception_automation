...
void controller::ros_setup() {
  ...
  ros::Subscriber request = nh_2.subscribe ("/magneto_autonomy/scan_request", 1, &controller::leg_sweep_start, this);
  std::cout << "Core Check 2 - Scan Request Subscriber Built" << std::endl;
  
  //Build the Event Handler Subscriber
  ros::Subscriber point5 = nh_2.subscribe ("/magneto_autonomy/calculate_target_trigger", 1, &controller::leg_sweep_end, this);
...
void controller::leg_sweep_start (const std_msgs::String::ConstPtr& message_data) {
  if (this->mapping == false) {
    ...
    if (this->debug == true) ROS_INFO_STREAM(current_leg);
    this->open_state = 1;
    this->active_map.camera_control();
  }
} 
