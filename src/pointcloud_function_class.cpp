...
void pointcloud_function_class::bounding_box (std::vector< std::vector<std::vector<float> >> coll_corners) {
  //bounding_pub = bounding_box_publisher.advertise<visualization_msgs::Marker> ( "/magneto_autonomy/bounding_boxes_marker", 1 );
  //Build objects. 
  //Build lines
...
if (this->debug == true) std::cout << "Building Optimal Points Markers.." << std::endl;
  ...
  //xyz_pub = perception_publisher.advertise< geometry_msgs::Vector3 >( "/magneto_autonomy/scan_result", 1);
  //slam_pub = perception_publisher.advertise<visualization_msgs::Marker>( "/magneto_autonomy/slam_map_beta", 1 );
  //vis_pub = perception_publisher.advertise<visualization_msgs::Marker>( "/magneto_autonomy/visualization_marker", 1 );    
